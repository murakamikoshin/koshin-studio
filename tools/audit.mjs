...
/* 遊ぶ先は worker/ が中継する。静的なサイトには無いので、
   リンク切れの数え上げからは外す。ただし works.json に
   書いてあるものだけ。書いていない /play/… は今まで通り拾う */
const PLAY_PATHS = (() => {
  const f = ROOT + '/data/works.json';
  if (!existsSync(f)) return [];
  try {
    return JSON.parse(readFileSync(f, 'utf8'))
      .map((w) => w.play).filter((p) => p && p.startsWith('/'));
  } catch (e) { return []; }
})();
if (PLAY_PATHS.length) console.log('中継で配る行き先（実体は別）: ' + PLAY_PATHS.join(' '));
...
for (const size of SIZES) {
  ...
  for (const path of PATHS) {
    ...
    if (size.n === 'pc') {
      /* リンク切れ。ただし遊ぶ先（/play/…）は Worker が中継するもので、
         ここには実体が無い。works.json に書いてある行き先だけを通す */
      for (const href of new Set(r.links)) {
        if (!href || /^(https?:|mailto:|tel:|#)/.test(href)) continue;
        if (PLAY_PATHS.some((p) => href === p || href.startsWith(p))) continue;
